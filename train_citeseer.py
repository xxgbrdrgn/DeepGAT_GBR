...
def random_splits(data, num_classes, lcc_mask):
    ...
    val_index = torch.cat([i[20:91] for i in indices], dim=0)

    rest_index = torch.cat([i[91:] for i in indices], dim=0)
...
def accuracy(out,data,mask):
    mask = data[mask]
    # マクネマー検定
    # file_path =r'C:\Users\xxgbr\Downloads\prog\DeepGAT-main\DeepGAT-main\mcnemar\citeseer2.xlsx'
...
def get_y_preds_loss(hs,data,cfg):
    ...
    #     y_pred_loss += F.nll_loss(y_pred[data.train_mask], data.y[data.train_mask]) * (cfg['loss_rate']+1)
    for h in hs:
        count=1
        h = h.mean(dim=1)
        y_pred = F.log_softmax(h, dim=-1)
...
def main(cfg):
    ...
    mlflow.set_experiment("citeseer")
    mlflow.start_run()
